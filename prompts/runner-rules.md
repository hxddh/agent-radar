...
- To **add a new** watchlist agent (or any new section), use `append` with a full `## AgentName` heading and its body — do NOT use `replace_section` with an invented anchor. A `replace_section` whose anchor does not exist is repaired, not refused: the runner first retargets it onto the one existing heading with the same title (numbering/case/markup ignored), and otherwise appends it as a new section at the same heading level, logging a warning either way. In `daily/YYYY-MM.md` only a `## YYYY-MM-DD` day-block anchor may be appended that way — use `append` for new day blocks.
...
- Keep the full model JSON response under **96,000 characters**; prefer compact `append` / `replace_section` updates.
...
18. **Community share** — at least 3 discussion-sourced bullets (Reddit/HN/Bluesky/Lobsters/dev.to cited) per day block, with 3–5 field reports in the User Workflow section; the runner counts them (`discussion_signal_count`, warned below 3, tracked in weekly By-the-Numbers). The screening pass runs the discussion shard first and reserves 3 discussion-backed user_workflow slots in the synthesis top-N.
