...
- **Coverage ledger (required)**: `#### 8. Assessment & Gaps` must contain a line `- Coverage ledger: checked=<lanes/vendors actually checked>; missed=<vendors not reachable>`. Gaps bullets only count as escape hatches when this ledger is present. If you omit it, the runner writes the ledger from collector telemetry (and adds any `Missing <class>:` line the evidence supports) — but your own ledger, naming what you actually checked, is more informative.
